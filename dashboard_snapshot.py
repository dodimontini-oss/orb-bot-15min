"""
One-off JSON snapshot for the live dashboard - works for both orb-bot-5min
and orb-bot-15min (identical script, deployed to both repos with only
BOT_ID/BOT_LABEL changed, same pattern as orb_live_bot.py). Reads Alpaca's
account, positions, and bracket orders, matches each bracket entry to its
REAL filled exit leg (see orb_trade_history.py's docstring for why naive
chronological fill-pairing is wrong here), and prints ONE json blob between
marker lines so it can be grepped out of the Action's log cleanly.
...
BOT_ID = "orb15min"  # the only line (with BOT_LABEL) that differs between the two repo copies
BOT_LABEL = "ORB 15-min (QQQ)"
